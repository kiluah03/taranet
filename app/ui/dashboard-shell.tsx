...
export function DashboardShell({ admin = false, children, userName, customerReference }: { admin?: boolean; children: React.ReactNode; userName?: string; customerReference?: string }) {
  ...
  return <div className="dash">
    <aside className={open ? "sidebar open" : "sidebar"}>
      ...
      <nav>{links.map(([Icon,label,href],i)=>admin?<button className={i===0?"active":""} key={label as string}><Icon size={18}/>{label as string}</button>:<Link className={i===0?"active":""} href={href as string} key={label as string} onClick={()=>setOpen(false)}><Icon size={18}/>{label as string}</Link>)}</nav>
      <div className="side-bottom"><div className="avatar">{admin?"AD":initials}</div><span><strong>{displayName}</strong><small>{admin?"Network administrator":customerReference || "Customer account"}</small></span>{admin?<LogOut size={17}/>:<form action="/auth/signout" method="post"><button aria-label="Sign out"><LogOut size={17}/></button></form>}</div>
    </aside>
