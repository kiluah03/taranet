...
## Password recovery and production URL repair ? 2026-09-23

- Login links to `/forgot-password`. Its CSRF-protected endpoint calls Supabase `resetPasswordForEmail` with the fixed production callback `/auth/callback?next=%2Freset-password`; it returns the same response for registered and unknown email addresses. Supabase enforces sending limits, and the UI adds a 60-second resend cooldown.
- The callback exchanges the PKCE code into HttpOnly cookies and opens `/reset-password`. Users must open the latest email link in the browser that requested it. Invalid or expired links return to the recovery form.
- Password updates require a server-verified Supabase user, matching passwords of at least 8 characters, and CSRF validation. The account comes from the session, never a submitted email or user ID. Successful updates request global signout. Session tokens and passwords are not returned to browser JavaScript or logged.
- The hosted project `jddqtvzyjhhrupksgkmr` had Site URL `http://localhost:3000` and an empty redirect allowlist. Corrected Site URL to `https://www.taranet.nz` and added `https://www.taranet.nz/auth/callback**`. Only these two settings changed; email confirmation and other hosted settings were preserved.
- Checks: `npm run test:auth` (22 tests) and `npm run build`. Tests cover CSRF, anti-enumeration responses, redirect restrictions, missing sessions, password confirmation, provider errors, session signout, and the installed SDK PKCE recovery cookie exchange.
